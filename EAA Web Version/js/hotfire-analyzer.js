...
function computeMetrics(targetThrust) {
    ...
    const useCustomSplice = customSpliceCheckbox && customSpliceCheckbox.checked;
    let mask;
...
async function createPlot(title, xData, yData, xLabel, yLabel, color, config = {}) {
    ...
    const chartConfig = _buildChartConfig(title, xyData, xLabel, yLabel, color);

    currentChart = new Chart(ctx2d, chartConfig);
}

function _buildChartConfig(title, xyData, xLabel, yLabel, color) {
    return {
        type: 'line',
        data: {
            datasets: [
 ...
 */
function _computeVenturiMdot(time, p1_psi, p2_psi, a1_in2, a2_in2, cd, y, rho) {
...
function calculateVenturiMdot() {
    ...
    const mdot = _computeVenturiMdot(time, p1_psi, p2_psi, a1_in2, a2_in2, cd, y, rho);
