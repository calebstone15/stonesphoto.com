...
function _createPlotConfig(title, xLabel, yLabel, xyData, color) {
    return {
        type: 'line',
        data: {
            datasets: [
...
async function createPlot(title, xData, yData, xLabel, yLabel, color, config = {}) {
    ...
    document.getElementById('avgDisplay').replaceChildren(avgSpan);
    ...
    const chartConfig = _createPlotConfig(title, xLabel, yLabel, xyData, color);
...
function handleChartClick(event, elements) {
    ...
    if (selectedPoints.length === 2) {
        ...
        const avgValSpan = document.createElement('span');
        avgValSpan.className = 'chart-avg-value';
        avgValSpan.textContent = `Average: ${avg.toFixed(3)} ${unit}`;
        ...
        avgDisplay.appendChild(avgValSpan);
...
function updateConstantLinesList() {
    ...
    customPlotConstantLines.forEach((line, i) => {
        ...
        textSpan.textContent = `${line.label}: ${line.value}${unitStr}`;
...
function openVenturiModal(type) {
    ...
    const defaultP1 = document.createElement('option');
    defaultP1.value = "";
    defaultP1.textContent = "-- Select P1 --";
    p1Select.appendChild(defaultP1);

    p2Select.replaceChildren();
    const defaultP2 = document.createElement('option');
    defaultP2.value = "";
    defaultP2.textContent = "-- Select P2 --";
    p2Select.appendChild(defaultP2);
...
function _calculateVenturiMdotMath(p1_psi, p2_psi, a1_in2, a2_in2, cd, y, rho) {
    // Convert areas to m²
    ...
    const mdot = p1_pa.map((p1, i) => {
        const p2 = p2_pa[i];
        const delta_p = Math.max(0, p1 - p2);  // Ensure non-negative
...
function calculateVenturiMdot() {
    ...
    const mdot = _calculateVenturiMdotMath(p1_psi, p2_psi, a1_in2, a2_in2, cd, y, rho);
